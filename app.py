...
from data_fetcher import fetch_stock_data, get_risk_free_rate
from visualizations import create_heatmap, create_greeks_plot, create_profit_loss_chart
...
def display_greeks(greeks, option_type):
    greek_symbols = {
        'delta': 'Δ (Delta)', 'gamma': 'Γ (Gamma)', 'vega': 'ν (Vega)', 'theta': 'Θ (Theta)', 'rho': 'ρ (Rho)'
    }
    
    greek_data = {
        greek_symbols[g.split('_')[0]]: f"{v:.4f}"
        for g, v in greeks.items() if g.endswith(option_type) or g == 'gamma' or g == 'vega'
    }
...
put_greeks_values = {greek: [BlackScholes(s, K, T, r, sigma, q).calculate_greeks()[greek] for s in S_range] for greek in ['delta_put', 'gamma', 'vega', 'theta_put', 'rho_put']}
col1, col2 = st.columns(2)
with col1:
